...
`bb run git setup` uses SSH remotes and recreates the workspace as eight primary clones plus nine linked worktrees for the language branches; existing repos are validated and updated with `git pull --ff-only origin <branch>`. `bb run git commit` runs `pi --print --model deepseek-v4-flash "commit and push"` in each dirty repo; `--root-dir DIR` targets somewhere other than the current directory.
