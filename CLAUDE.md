...
- `bb run git setup [--dry-run] [--root-dir DIR]` — clone/update the expected workspace with SSH remotes, five primary clones, and linked worktrees for sibling language branches; existing repos are validated and pulled with `--ff-only origin <branch>`.
